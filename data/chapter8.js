...
window.CHAPTER8 = { events: [
  ...
  // 队长选举（选择你的姿态）
  {
    ...
    text: [
      ...
      "没人动。赵凛抱臂，嘴角有一丝冷笑。林啸仰头看天。阿贵用胳膊肘捅了捅你的肋骨。",
    ...
    choices: [
      { id: "A", text: "出列自荐：我来当这个队长", check: { attrs: ["resolve", "pressure"], difficulty: 42, tag: "决断+抗压" }, next: "ch8_group1",
        success: { text: "你往前迈了一步。草皮在鞋底发出轻微的声响。赵凛挑了下眉，没吭声。姜太虚看了你两秒，点了下头：「行。扛住了再说。」", effects: { reputation: 8, attrs: { resolve: 1 }, flags: { isCaptain: true } } },
      ...
      { id: "B", text: "推举赵凛：让宿敌扛旗，你辅佐", check: { attrs: ["iq", "passing"], difficulty: 38, tag: "球商+传球" }, next: "ch8_group1",
        success: { text: "你朝赵凛努了努嘴。他愣了半秒，弯腰捡起袖标，什么都没说。但你看见他系袖标时手指收紧了一下。你退后半步，开始串联。", effects: { reputation: 5, bonds: { zhaolin: 12 }, attrs: { iq: 1 } } },
        fail: { text: "赵凛没接袖标，声音很平：「不用让。」空气冷了两秒。", effects: { stamina: -3 } }
      },
  ...
  // 小组赛第1场（以强打弱）
  {
    ...
    fallback_choices: [
      ...
      { id: "B", sit: "balanced", text: "演练传控，磨合阵容", check: { attrs: ["passing", "vision"], difficulty: 38, tag: "传球+视野" },
        success: { text: "你与林啸、苏晚连续三脚传递，球不落地，对手连球皮都摸不着。最后一脚直塞，队友推射空门。", effects: { reputation: 7, assists: 1, bonds: { linxiao: 8, suwan: 8 }, attrs: { passing: 1 } } },
  ...
  // 战术分歧（队内矛盾·选择站队）
  {
    ...
    text: [
      ...
      "赵凛把笔往桌上一搁：「关键球，给我。一个人解决。」林啸猛地站起来，椅子刮过地板：「足球是十一个人的。」两个人对视，空气像绷紧的弦。",
      "姜太虚一直没说话。他合上战术本，看向你：「你说。怎么踢。」"
    ],
    choices: [
      { id: "A", text: "支持团队：足球是十一个人的", check: { attrs: ["iq", "passing"], difficulty: 40, tag: "球商+传球" }, next: "ch8_quarter",
        success: { text: "你把训练赛里那些跑通的配合一帧帧说给众人听。房间里慢慢安静下来。赵凛靠回椅背，沉默了很久，最后点了下头：「行。证明给我看。」", effects: { reputation: 6, bonds: { linxiao: 10 }, attrs: { iq: 1 }, flags: { teamFirst: true } } },
      ...
      { id: "B", text: "支持赵凛：关键球就该交给最强的人", check: { attrs: ["resolve", "shooting"], difficulty: 40, tag: "决断+射门" }, next: "ch8_quarter",
        success: { text: "你看向赵凛：「关键球，给他。」赵凛没笑，只是把笔重新拿起来，在战术板上画了条线。你们之间不需要多余的话。", effects: { reputation: 5, bonds: { zhaolin: 12 }, attrs: { resolve: 1 } } },
        fail: { text: "林啸把椅子踢开，门摔得整面墙都在震。裂痕，已经在了。", effects: { stamina: -4, demonValue: 2 } }
      },
  ...
  // 四分之一决赛
  {
    ...
    result: {
      ...
      lose:    { text: "八强战折戟。你坐在草皮上，膝盖的擦伤渗着血。全国大赛，到此为止。", effects: { reputation: -5, stamina: -6, demonValue: 5 } }
    },
    next: "ch8_semi"
  },
  ...
  // 半决赛·新宿敌（土灵根中锋·石破岳，"不动如山"克制冲击）
  {
    ...
    result: {
      ...
      lose:    { text: "你输了。石破岳从你身边走过，停了一秒：「山不是用来撼的。」然后他继续走，没回头。", effects: { reputation: -6, stamina: -6, demonValue: 6 } }
    },
    next: "ch8_final"
  },
  ...
  // 决赛·BOSS（卫冕冠军·天阙皇朝，胜利设 nationalChamp）
  {
    ...
    text: [
      ...
      "赵凛活动手腕，骨节咔咔响。林啸弯腰系紧鞋带。阿贵闭眼深呼吸。你望向看台最高处——矿坑的方向。这一战，不为自己。"
    ],
    ...
    fallback_choices: [
      ...
      { id: "B", sit: "balanced", text: "团队配合，撕开王者防线", check: { attrs: ["passing", "vision"], difficulty: 46, tag: "传球+视野" },
        success: { text: "你与赵凛、林啸三角传递，三脚球，防线撕开。助攻队友推射入网。十一个人的剑，刺穿了王者的甲。", effects: { reputation: 12, assists: 1, bonds: { zhaolin: 10, linxiao: 10 }, attrs: { passing: 1 } } },
  ...
  // 合同蜂拥（名利场初现）
  {
    ...
    text: [
      ...
      "孙先生摘下老花镜，用指节敲了敲桌面：「记住。合同签的不是钱。是你接下来五年，每天醒来要面对的东西。」"
    ],
    choices: [
