...
window.CHAPTER7 = { events: [
  ...
  // ===== 事件2：第一场·金阙院风格（match·金） =====
  {
    ...
    result: {
      ...
      win:     { text: "你赢了，{lastScore}。金阙院的铁壁，你凿穿了。哨响的时候你跪在地上。", effects: { reputation: 12 } },
      draw:    { text: "平了，{lastScore}。金阙院，你没输，但也没赢。铁壁就是铁壁。", effects: { reputation: 5 } },
      lose:    { text: "{lastScore}，输了。不是被碾压，是被闷死了。他们不攻，他们只是等。等你失误，然后，一刀。", effects: { demonValue: 3, reputation: 2 } }
    },
    next: "ch7_bulan"
  },
  ...
  // ===== 事件3：第三场·布澜院风格（match·水） =====
  {
    ...
    result: {
      ...
      win:     { text: "你赢了，{lastScore}。他们笑着踢，你咬着牙踢。你赢了。", effects: { reputation: 10 } },
      draw:    { text: "平了，{lastScore}。他们进了两个，你也进了两个。那个进球，你过了两个人，{companion1Name}传的。", effects: { reputation: 5, goals: 1 } },
      lose:    { text: "{lastScore}，输了。但你进了一个。那个进球，你过了两个人，{companion1Name}传的。升班马进球，够了。", effects: { reputation: 5, goals: 1 } }
    },
    next: "ch7_firstwin"
  },
  ...
  // ===== 事件5：传奇登场（match·赤焰院+传奇） =====
  {
    ...
    result: {
      ...
      win:     { text: "你赢了，{lastScore}。传奇在场，你赢了。他经过你的时候，「不错。」两个字，比什么都重。", effects: { reputation: 12, flags: { legendEncounter: true } } },
      draw:    { text: "平了，{lastScore}。传奇进了一个，你也进了一个。他经过你的时候，「不错。」", effects: { reputation: 8, flags: { legendEncounter: true } } },
      lose:    { text: "{lastScore}，被逆转了。德罗巴进了一个，助攻了一个。你站在场中央，看着他走回替补席。他经过你的时候，停了一下。「不错。」他说，然后走了。", effects: { reputation: 5, flags: { legendEncounter: true } } }
    },
    next: "ch7_midseason"
  },
  ...
  // ===== 事件8：第十场·宿敌复仇（match） =====
  {
    ...
    result: {
      ...
      win:     { text: "{lastScore}，你赢了。第一次，你赢了沈惊寒。他走过来，伸出手。「下次。」他说。你握了。「随时。」", effects: { reputation: 10, bonds: { sudi: 2 }, flags: { rivalFirstWin: true } } },
      draw:    { text: "平了，{lastScore}。你和他，面对面，谁也没赢。他看了你一眼，笑了。「下次。」", effects: { reputation: 5, bonds: { sudi: 1 } } },
      lose:    { text: "输了，{lastScore}。他过了你，射了，进了。他朝你看了一眼，笑了。「下次。」", effects: { demonValue: 5, bonds: { sudi: 1 } } }
    },
    next: "ch7_endseason"
  },
