...
window.CHAPTER4 = { events: [
  ...
  // ===== 事件7：第八场·生死（比赛·6节点） =====
  {
    ...
    result: {
      ...
      win:     { text: "哨响。2-1。赢了。你们五个抱在一起。{companion1Name}在吼。后卫在哭。四强。", effects: { reputation: 10, flags: { campSemifinal: true } } },
      draw:    { text: "平了。1-1。不够。但也没输。最后一场。还得拼。", effects: { reputation: 3, demonValue: 3 } },
      lose:    { text: "输了。0-2。你坐在场边。腿在抖。不是累。是怕。看别人脸色的滋味。你不想再尝。", effects: { reputation: -3, demonValue: 6 } }
    },
    next: "ch4_semi"
  },
  ...
  // ===== 事件8：半决赛（比赛·BOSS战） =====
  {
    ...
    result: {
      ...
      win:     { text: "赢了。2-1。绝杀。终场哨响的时候你跪在地上。通脉境又怎样。你赢了。", effects: { reputation: 15, flags: { campFinal: true } } },
