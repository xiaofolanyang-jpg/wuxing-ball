 ...
 * 事件链：开赛通知 → 小组赛首战 → 赛前集训 → 宿敌对决(武石) → 队长袖标 → 决赛 → 假期抉择 → 结局分发
 * 说明：承接第二章《崭露》的职业线，对应设计稿第四章"市联赛·下/省赛"的浓缩延续。
 *       宿敌对决同时触发"既生瑜何生亮"(武石)与"水火不容"(水灵根)羁绊进度。
 */
window.CHAPTER3 = { events: [
  ...
  // 开场
  {
    ...
    text: [
      ...
      "「省赛的通知下来了。」他把报名名单推过来，手指点了点其中一个名字，「武石，东城实验。」顿了一下，又补了句，「你俩那笔账……该上球场算了。」"
    ],
  ...
  // 小组赛首战（比赛）
  {
    ...
    fallback_choices: [
      { id: "A", sit: "attack", text: "接球转身，不犹豫，直接起脚", check: { attrs: ["shooting", "burst"], difficulty: 28, tag: "射门+爆发" },
      ...
      { id: "B", sit: "balanced", text: "不贪功，直塞找插上的队友", check: { attrs: ["passing", "vision"], difficulty: 25, tag: "传球+视野" },
      ...
      { id: "C", sit: "defense", text: "先落位回防，把后场站稳", check: { attrs: ["tackle", "positioning"], difficulty: 20, tag: "铲断+站位" },
  ...
  // 赛前集训（修炼）
  {
    ...
    chapter: 3,
    type: "train",
    text: "宿敌对决前两天，球队封闭集训。傍晚的球场灯光把影子拉得很长，你一个人留在训练场加练任意球，脚背抽了又抽，直到小腿肌肉开始发酸。武石不好对付，你心里清楚。多练一脚，到时候就多一分底气。夜里回到宿舍，你把灵力一遍遍压进经脉，窗外有虫鸣。",
    next: "ch3_rival_match"
  },

  // 宿敌对决（比赛·武石·水灵根 → 触发既生瑜何生亮+水火不容羁绊）
  {
    id: "ch3_rival_match",
    chapter: 3,
    type: "match",
    text: "省赛半决赛。你站在中圈等开球的时候，隔着四十米看见武石在对面活动脚踝。他没看你，但你知道他在看你。三个月了。他嘴角动了一下，像是说了句什么。后来你才知道那是：「等你很久了。」哨响。",
    opponent: { name: "东城实验·武石", element: "水", strength: 48 },
    fallback_choices: [
      { id: "A", sit: "attack", text: "不绕路，正面起脚", check: { attrs: ["shooting", "resolve"], difficulty: 35, tag: "射门+决断" },
        success: { text: "你{elementAdj}地拔脚，球从武石伸出的腿边擦过去，砸进网窝。他没回头，但你看见他肩膀绷紧了。", effects: { reputation: 14, goals: 1, attrs: { shooting: 1 } } },
        fail: { text: "武石的预判比你快半步，腿一伸，球弹出去。他甚至没看你一眼。", effects: { stamina: -6 } },
        critical: { text: "【灵光一闪】球进了。全场炸了。武石站在原地，脸色很难看，但他鼓了两下掌。", effects: { reputation: 22, goals: 1, attrs: { shooting: 2 }, flags: { keySuccess: true } } }
      },
      { id: "B", sit: "balanced", text: "把武石引过来，分球给空位的队友", check: { attrs: ["passing", "vision"], difficulty: 30, tag: "传球+视野" },
        success: { text: "武石果然上抢，你等的就是这个。脚腕一抖，球到了无人盯防的队友脚下，他笑纳了。", effects: { reputation: 10, assists: 1, attrs: { passing: 1 } } },
        fail: { text: "武石没上抢。他卡住了传球线路，像早就知道你要往哪送。", effects: { stamina: -5 } }
      },
      { id: "C", sit: "defense", text: "贴住武石，不让他舒服拿球", check: { attrs: ["tackle", "strength"], difficulty: 32, tag: "铲断+对抗" },
        success: { text: "你全场贴着武石，肩膀挨着肩膀。他拿不到舒服的球，越踢越躁。", effects: { reputation: 8, attrs: { tackle: 1 } } },
        fail: { text: "武石一个沉肩变向，你重心跟丢了。他头也不回地往前带。", effects: { stamina: -6 } }
      }
    ],
    result: {
      bigwin: { text: "大胜。武石走过来的时候你以为他要握手，他只是盯着你看了两秒：「没完。」然后转身走了。", effects: { reputation: 16, flags: { keySuccess: true } } },
      win:     { text: "赢了。武石一句话没说，把护腿板塞进包里，拉链拉得很快。", effects: { reputation: 10 } },
      draw:    { text: "平局。武石擦着汗经过你身边，语气很平：「你还是赢不了我。」", effects: { reputation: 4 }, next: "ch3_eliminated" },
      lose:    { text: "输了。武石没有庆祝，只是走过你身边时停了半步：「你变弱了。」那三个字比输球本身更扎人。", effects: { reputation: -5, stamina: -6, demonValue: 5 }, next: "ch3_eliminated" }
    },
    next: "ch3_coach_talk"
  },

  // 半决赛出局（淘汰赛输球/未胜 → 止步，不进入决赛）
  {
    id: "ch3_eliminated",
    chapter: 3,
    text: [
      "省赛止步半决赛。回程的大巴上，你靠着窗，看路灯一盏盏往后退。脚背上的伤疤还在痒，像是这场比赛留下的最后一句话。",
      "教练没多说，只在下车时拍了拍你的肩。几天后，他从抽屉里摸出一份烫金请柬递给你，语气跟递瓶水似的：「省赛翻篇了。学院大比，全国八所学院的尖子都去。那才是真正踢球的地方。」",
      "你接过请柬，看了很久。输掉的账，换个地方算。"
    ],
    system: "【第三章·省赛 完（止步半决赛）。接下来：第四章·学院大比·组队。】",
  ...
  // 决赛（比赛）
  {
    ...
    fallback_choices: [
      { id: "A", sit: "attack", text: "禁区前沿，抡一脚", check: { attrs: ["shooting", "power"], difficulty: 33, tag: "射门+力量" },
      ...
      { id: "B", sit: "balanced", text: "不急，跟队友打配合渗透", check: { attrs: ["passing", "iq"], difficulty: 28, tag: "传球+球商" },
      ...
      { id: "C", sit: "defense", text: "先守住，机会会来的", check: { attrs: ["positioning", "balance"], difficulty: 22, tag: "站位+平衡" },
