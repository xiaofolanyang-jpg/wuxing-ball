 ...
 * 事件链：大比开幕 → 小组赛×2 → 1/4决赛 → 半决赛(宿敌武石) → 决赛前夜 → 决赛(BOSS) → MVP结算 → 合同抉择 → 章末
 * 说明：5v5团队赛，个人影响力极大。选人结果影响难度，羁绊密集推进。
 *       半决赛触发"既生瑜何生亮"(武石)；决赛以弱打强。大比排名决定声望。
 */
window.CHAPTER5 = { events: [
  ...
  // 开场：大比开幕
  {
    ...
    text: [
      ...
      "找到了。武石站在对面那支队伍的阵前，隔着半个球场。他也在看你。嘴唇动了一下，没出声，但你读出来了：「决赛见。」"
    ],
  ...
  // 小组赛第1场（以强打弱）
  {
    ...
    chapter: 5,
    type: "match",
    text: "小组赛首轮，南疆学院。实力不算强，但孙德斯赛前还是叮嘱了句：「大比没有弱队，别松。」他说完就坐回折叠椅上去了。",
    opponent: { name: "南疆学院", element: "木", strength: 40 },
    fallback_choices: [
      { id: "A", sit: "attack", text: "开场就抢，不给他们站稳", check: { attrs: ["shooting", "burst"], difficulty: 33, tag: "射门+爆发" },
      ...
      { id: "B", sit: "balanced", text: "跟苏雯打传控，慢慢撕", check: { attrs: ["passing", "vision"], difficulty: 31, tag: "传球+视野" },
        success: { text: "你和苏雯连续三脚传递，防线被扯开了，队友插上笑纳。苏雯连庆祝都省了，直接跑回中圈。", effects: { reputation: 7, assists: 1, bonds: { suwan: 8 }, attrs: { passing: 1 } } },
        fail: { text: "第四脚传大了，被断。对方直接打反击，你回追了三十米。", effects: { stamina: -4 } }
      },
      { id: "C", sit: "defense", text: "不急，先把节奏控住", check: { attrs: ["rhythm", "positioning"], difficulty: 25, tag: "节奏+站位" },
    ...
    result: {
      ...
      draw:    { text: "平了。孙德斯从折叠椅上站起来，脸色不太好看：「我说了，大比没有弱队。」", effects: { reputation: 1 } },
      lose:    { text: "输了。出线形势一下子紧了。更衣室里范志贵把水瓶摔了，没人怪他。", effects: { reputation: -4, stamina: -5, demonValue: 4 } }
    },
    next: "ch5_group2"
  },
  ...
  // 小组赛第2场·出线关键战（势均力敌，关键时刻×2）
  {
    ...
    chapter: 5,
    type: "match",
    text: "小组赛次轮，出线关键战。东海学院，实力跟你队差不多，阵中那个水灵根门将是老熟人——当年布澜青训出来的。赛前你活动脚踝的时候看见他在对面热身，扑救动作还是那么舒展。",
    opponent: { name: "东海学院", element: "水", strength: 48 },
    fallback_choices: [
      { id: "A", sit: "attack", text: "不绕弯子，正面抽射", check: { attrs: ["shooting", "power"], difficulty: 39, tag: "射门+力量" },
      ...
      { id: "B", sit: "balanced", text: "佯装传中，搓射远角", check: { attrs: ["shooting", "iq"], difficulty: 35, tag: "射门+球商" },
      ...
      { id: "C", sit: "defense", text: "先冻结他们的核心，再想办法", check: { attrs: ["tackle", "iq"], difficulty: 33, tag: "铲断+球商" },
    ...
    result: {
      ...
      win:     { text: "险胜。终场哨响，五个人抱在一起。范志贵喊得最大声。", effects: { reputation: 7 } },
  ...
  // 四分之一决赛
  {
    ...
    chapter: 5,
    type: "match",
    text: "八强战，西岭学院。土灵根铁桶阵，后防线跟城墙似的。孙德斯赛前布置的时候说了句：「别急。用耐心磨他们的耐心。」",
    opponent: { name: "西岭学院", element: "土", strength: 50 },
    fallback_choices: [
      { id: "A", sit: "attack", text: "定位球抢点，头槌砸", check: { attrs: ["heading", "positioning"], difficulty: 37, tag: "头球+站位" },
      ...
      { id: "B", sit: "balanced", text: "肋部直塞，打身后", check: { attrs: ["passing", "vision"], difficulty: 35, tag: "传球+视野" },
        success: { text: "你{elementAdj}的直塞从两个后卫中间穿过去，内牛尔的速度这时候就是武器，他插上就是一脚。", effects: { reputation: 8, assists: 1, bonds: { linxiao: 8 }, attrs: { passing: 1 } } },
        fail: { text: "直塞被土灵根后卫预判了，伸脚一挡。人家的站位确实老到。", effects: { stamina: -4 } }
      },
      { id: "C", sit: "defense", text: "不急，等他们先犯错", check: { attrs: ["rhythm", "pressure"], difficulty: 29, tag: "节奏+抗压" },
    ...
    result: {
      ...
      win:     { text: "小胜，晋级四强。更衣室里范志贵把毛巾甩得跟直升机似的。", effects: { reputation: 6 } },
      draw:    { text: "平了，加时赛，点球。赢是赢了，但腿已经不是自己的了。", effects: { reputation: 4, stamina: -8 } },
      lose:    { text: "八强，到头了。你坐在更衣室的长凳上，盯着鞋钉上的泥看了很久。", effects: { reputation: -4, stamina: -6, demonValue: 5 }, next: "ch5_mvp" }
    },
    next: "ch5_semi"
  },

  // 半决赛·宿敌对决（武石的队，"既生瑜何生亮"触发）
  {
    id: "ch5_semi",
    chapter: 5,
    type: "match",
    text: [
      "半决赛。北都书院，武石的队。你们一路横扫进来的，气势正盛。",
      "赛前握手的时候，武石的手掌很干，握力很大。他活动着手腕，水灵力在指尖流转，语气很平：「从青训到省赛，再到大比。该有个了断了。」",
      "解说席上有人感慨：「这一代最好的两个，偏偏赶上了同一届。」你听见了，没当回事。哨响了。"
    ],
    opponent: { name: "北都书院·武石", element: "水", strength: 54 },
    fallback_choices: [
      { id: "A", sit: "attack", text: "正面来，用最强的一脚回应", check: { attrs: ["shooting", "resolve"], difficulty: 41, tag: "射门+决断" },
        success: { text: "你{elementAdj}地拔脚，球从武石伸出的腿边擦过去。进了。他站在原地没动，肩膀绷得很紧。", effects: { reputation: 13, goals: 1, bonds: { zhaolin: 15 }, attrs: { shooting: 1 } } },
        fail: { text: "武石的预判又快了半步。他挡完球之后看了你一眼，那眼神说不上是轻蔑还是认真。", effects: { stamina: -6 } },
        critical: { text: "【灵光一闪】那脚球后来被叫作世界波。你只记得触球那一下脚背发麻，然后全场站起来了。武石鼓了两下掌。", effects: { reputation: 22, goals: 1, bonds: { zhaolin: 20 }, attrs: { shooting: 2 }, flags: { keySuccess: true } } }
      },
      { id: "B", sit: "balanced", text: "把武石引过来，给队友做嫁衣", check: { attrs: ["passing", "iq"], difficulty: 37, tag: "传球+球商" },
        success: { text: "武石果然包夹过来了。你等的就是这个，脚腕一抖，球到了空位队友脚下。他笑纳了。", effects: { reputation: 9, assists: 1, bonds: { zhaolin: 10 }, attrs: { passing: 1 } } },
        fail: { text: "武石没上抢。他卡住了线路，像早就知道你要往哪送。", effects: { stamina: -5 } }
      },
      { id: "C", sit: "defense", text: "贴住他，不让他舒服拿球", check: { attrs: ["tackle", "strength"], difficulty: 39, tag: "铲断+对抗" },
        success: { text: "你全场贴着武石，肩膀挨着肩膀。他越踢越躁，有一次甚至朝裁判吼了两句。", effects: { reputation: 8, bonds: { zhaolin: 10 }, attrs: { tackle: 1 } } },
        fail: { text: "武石一个沉肩变向，你重心跟丢了。他往前带的时候头也没回。", effects: { stamina: -6 } }
      }
    ],
    result: {
      bigwin: { text: "大胜。武石走过来的时候你以为他要握手。他没有。他只是盯着你看了两秒，一字一句：「决赛，或者下一次。我不会再输。」", effects: { reputation: 14, bonds: { zhaolin: 15 }, flags: { keySuccess: true } } },
      win:     { text: "赢了。武石一句话没说，把护腿板塞进包里。他离场的时候背影很直，但步子很快。", effects: { reputation: 10, bonds: { zhaolin: 10 } } },
      draw:    { text: "平了，点球输了。武石擦着汗经过你身边：「你还是赢不了我。」语气很平，像在陈述事实。", effects: { reputation: 3, demonValue: 4 }, next: "ch5_mvp" },
      lose:    { text: "输了。武石没有庆祝，只是走过你身边时停了半步：「决赛的门票，我收下了。」", effects: { reputation: -6, stamina: -6, demonValue: 6 }, next: "ch5_mvp" }
    },
    next: "ch5_final_eve"
  },
  ...
  // 决赛前夜·队友夜谈（羁绊大推进）
  {
    ...
    text: [
      ...
      "内牛尔靠着栏杆，难得地先开了口：「以前我觉得踢球是一个人的事。」范志贵接话：「现在呢？」内牛尔没马上回答，瞥了你一眼，过了几秒才说：「……五个人的。」",
      "苏雯坐在地上，膝盖抱着，轻声说了句：「明天不管输赢，咱们踢的是自己的球。」没人接话。夜风把范志贵的头发吹乱了，他也没去拨。五个人就这么坐着，谁也没走。"
    ],
  ...
  // 决赛（BOSS战，以弱打强）
  {
    ...
    chapter: 5,
    type: "match",
    text: "决赛。金阙皇朝，卫冕冠军，全员凝形境以上。赛前没有一个人看好你们，赔率一边倒。孙德斯倒是不在乎，他靠在更衣室的柜子上，剥了颗薄荷糖：「光脚的不怕穿鞋的。去，把他们的王座掀了。」语气跟说「去食堂吃饭」似的。",
    opponent: { name: "金阙皇朝", element: "金", strength: 60 },
    fallback_choices: [
      { id: "A", sit: "attack", text: "反击中自己带，单骑闯关", check: { attrs: ["dribble", "burst"], difficulty: 43, tag: "盘带+爆发" },
      ...
      { id: "B", sit: "balanced", text: "五个人一起打，层层渗透", check: { attrs: ["passing", "vision"], difficulty: 39, tag: "传球+视野" },
      ...
      { id: "C", sit: "defense", text: "先守住，拖到点球再说", check: { attrs: ["tackle", "pressure"], difficulty: 35, tag: "铲断+抗压" },
  ...
  // MVP结算 + 合同抉择
  {
    ...
    text: [
      ...
      "孙德斯靠在门框上，剥了颗薄荷糖：「还有条路。」他顿了顿，「下个月，淬炼营选拔。三十六人进，十一人出。那才是通往天罡联队的独木桥。」他说这话的时候语气很平，但你听出了底下的意思。"
    ],
    choices: [
